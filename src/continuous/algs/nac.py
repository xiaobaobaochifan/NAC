'''
This is the main module to conduct NAC policy learning, with both state space and action space 
to be continuous. We first use offline data to learn the best candidate policy by nac_learn,
...
# We need another function named 
...
def offline_collection(offline_data, env_fn, ac_old, size, start_steps, max_ep_len=1000):
    ...
    for t in range(size):
        ...
        # # The 'done' flag at the step of hitting the epsiode horzon should not be treated
        # # as an actual done tuple
...
def nac_learn(env_fn, ac_old, cost_fn, o_initial, offline_data, old_ref,
              ...
              gamma=0.99, polyak=0.995, lr=3e-4, batch_size=256, act_gen_runs=1000,
    ...
    # A safer way to compute the universal maximum magnitude of any action variable
    act_limit = max(np.maximum(np.abs(env.action_space.high), np.abs(env.action_space.low)))
    ...
    # # Fill replay buffer with actualy offline data
    # offline_collection(offline_data, env_fn, ac_old, offline_size, start_steps, max_ep_len)

    # Function to compute NAC net Q-losses
    def compute_loss_nq(data, cost_fn, ac_old, ac_new, ac_targ):
        ...
        with torch.no_grad():
            ...
            nq1_targ = ac_targ.q1(o2, a2)
            nq2_targ = ac_targ.q2(o2, a2)
    ...
    def compute_loss_pi(o_initial, cost_fn, ac_old, ac_new, act_gen_runs):
        ...
        # Prepare o for batch operation
        # Repeat the state to create a batch
        o = o_initial.repeat(act_gen_runs, 1)  

        # Generate actions from the current policy for the batched states
        # actions = ac_new.act(o)
        # actions = torch.as_tensor(actions, dtype=torch.float32)
        actions, _, _ = ac_new.pi(o)

        # net Q-values
        cost = cost_fn(ac_old, ac_new)        
        nq1 = ac_new.q1(o, actions) - cost
        nq2 = ac_new.q2(o, actions) - cost

        # Conservative estimates
        q_pi = torch.min(nq1, nq2)

        # Policy loss normalized to mean
        loss_pi = - q_pi.mean()
    ...
    def update(data, cost_fn, ac_old, ac_new, ac_targ):
        ...
        # Insert this into the main flow before calculating the gradident of net Q-losses
        ...
        loss_nq = compute_loss_nq(data, cost_fn, ac_old, ac_new, ac_targ)
        loss_nq.backward()
        ...
        loss_pi = compute_loss_pi(o_initial, cost_fn, ac_old, ac_new, act_gen_runs)
        loss_pi.backward()
    ...
    def test_agent(test_env, num_test_episodes, max_ep_len):
        for _ in range(num_test_episodes):
            ...
            while not(trm or trc or (ep_len == max_ep_len)):
                ...
                o, r, trm, trc, stepinfo = test_env.step(get_action(o, True))
                ep_ret += r
                ep_len += 1
